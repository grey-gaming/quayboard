  ...
  <div className="min-w-0 overflow-hidden">
    <ReactMarkdown
      components={{
        ...
        h2: ({ children }) => (
          <h2 className="mt-8 break-words border-t border-border/80 pt-5 font-display text-[1.35rem] font-semibold tracking-[-0.02em]">
            {children}
          </h2>
        ),
        h3: ({ children }) => <h3 className="mt-6 break-words text-lg font-semibold">{children}</h3>,
        hr: () => <hr className="border-border/80" />,
        li: ({ children }) => <li className="break-words leading-7 text-foreground">{children}</li>,
        ol: ({ children }) => <ol className="ml-6 list-decimal space-y-2">{children}</ol>,
        p: ({ children }) => <p className="break-words leading-7 text-secondary">{children}</p>,
        pre: ({ children }) => <pre className="my-4 max-w-full overflow-x-auto">{children as ReactNode}</pre>,
        table: ({ children }) => (
